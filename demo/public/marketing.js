...
// PromptApi and usePromptApi are now auto-imported.`
  ...
  const setAvailability = (id, status) => {
    ...
    badge.replaceChildren(dot, document.createTextNode(` ${availabilityLabel(status)}`));
  ...
  const checkNativeApis = () => {
    ...
    setAvailability('webmcp', document.modelContext ? 'available' : 'unavailable');
  ...
  const setupHeroMotion = () => {
    ...
    if (!(hero instanceof HTMLElement) || hero.dataset.motionReady === 'true') return;
