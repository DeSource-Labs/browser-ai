...
const cloneChat = (chat: AiChatRecord): AiChatRecord => ({
  ...
  summaries: Array.isArray(chat.summaries)
    ? chat.summaries.map(cloneSummary)
    : [],
});
...
const normalizeMessages = (
  ...
  const messages = rawMessages.map((rawMessage, index) => {
    ...
    const fallbackTimestamp =
      previousTimestamp + Math.max(index === 0 ? 0 : 1000, 1);
...
const normalizeSummaries = (
  ...
  rawSummaries.forEach((rawSummary) => {
    ...
    const startIndex = Number.isInteger(entry.startIndex)
      ? (entry.startIndex as number)
      : -1;
    const endIndex = Number.isInteger(entry.endIndex)
      ? (entry.endIndex as number)
      : -1;
    ...
    const level =
      Number.isInteger(entry.level) && entry.level! >= 0 ? entry.level! : 0;
    ...
    const id =
      typeof entry.id === "string" && entry.id.length > 0
        ? entry.id
        : generateId();
    const createdAt = isValidTimestamp(entry.createdAt)
      ? entry.createdAt
      : Date.now();
    const updatedAt = isValidTimestamp(entry.updatedAt)
      ? entry.updatedAt
      : createdAt;
...
const normalizeChatRecord = (
  ...
  const toolValue =
    typeof source.tool === "string" && source.tool.length > 0
      ? source.tool
      : tool;
  ...
  const lastMessageTimestamp =
    normalizedMessages.messages[normalizedMessages.messages.length - 1]
      ?.timestamp ?? createdAt;

  let updatedAt = isValidTimestamp(source.updatedAt)
    ? source.updatedAt
    : Math.max(createdAt, lastMessageTimestamp);
...
export function useAiChats(tool: AiChatTool) {
  ...
  const loadChats = async () => {
    ...
    const normalized = records.map((record) =>
      normalizeChatRecord(record, tool),
    );
