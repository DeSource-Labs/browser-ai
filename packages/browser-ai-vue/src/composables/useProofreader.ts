...
import { normalizeTextInput, stripHtmlForText, type TextChunk } from '../utils/text';
import {
...
export function useProofreader(defaultCreateOptions: ProofreaderCreate = {}) {
  const proofreader = ref<Proofreader | null>(null);
  const availability = ref<Availability | null>(null);
  const createOptions = ref<ProofreaderCreate>(normalizeCreateOptions(defaultCreateOptions));
  ...
  const progressState = ref<ProofreaderProgressState>(createEmptyProgressState());
  ...
  const checkAvailability = async (options: ProofreaderCreateCoreOptions = createOptions.value) => {
    return safeCheckAvailability(getProofreader(), normalizeCreateOptions(options));
  };

  const requestAvailability = async (options: ProofreaderCreateCoreOptions = createOptions.value) => {
  ...
  const init = async (options: ProofreaderCreateCoreOptions = createOptions.value) => {
    destroy();
  ...
  const ensureProofreader = async (options?: ProofreaderCreate, autoCreate = true) => {
    const nextOptions = normalizeCreateOptions(options ?? createOptions.value);

    if (proofreader.value && getCreateOptionsKey(nextOptions) === getCreateOptionsKey(createOptions.value)) {
      return proofreader.value;
    }
  ...
  const measureInputUsage = async (
    ...
    const instance = await ensureProofreader(options.createOptions, options.autoCreate !== false);
    ...
    setProgressState({ phase: 'measuring', inputQuota: getInputQuota(instance) }, options.onProgress);
    const signal = operation.begin();
  ...
  const proofreadWithDetails = async (
    ...
    try {
      ...
      const measuredBudget = inputQuota.value && Number.isFinite(inputQuota.value)
        ? Math.floor(inputQuota.value * chunkBudgetRatio)
        : null;
      const shouldChunkByUsage = largeInputStrategy !== 'never'
        && measuredBudget != null
        && usage != null
        && usage > measuredBudget;
      const shouldChunkByLength = largeInputStrategy !== 'never'
      ...
      setProgressState({ phase: 'chunking', chunked: true }, options.onProgress);
  ...
  const proofreadMany = async (
    ...
    for (let index = 0; index < items.length; index += 1) {
      ...
      try {
        batchResults.push(await proofreadWithDetails(item.input, {
          ...runOptions,
          createOptions: {
            ...baseCreateOptions,
