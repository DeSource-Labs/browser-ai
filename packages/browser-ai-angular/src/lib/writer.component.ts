...
@Component({
  ...
  template: `
    <browser-ai-text-tool
      ...
      [(value)]="value"
      [result]="resultText() || 'Generated draft will appear here.'"
    ...
    >
      <div browserAiOutput>
        @if (editor.configuration().createOptions['format'] === 'plain-text') {
          <pre>{{ resultText() || 'Generated draft will appear here.' }}</pre>
        } @else {
          <browser-ai-markdown [content]="resultText() || 'Generated draft will appear here.'" />
        }
      </div>
    </browser-ai-text-tool>
  `
})
export class BrowserAiWriterComponent {
  readonly value = model('');
  readonly options = input<Omit<WriterCreateOptions, 'monitor' | 'signal'>>({});
  readonly runOptions = input<WriterRunOptions>({});
  readonly autoInit = input(true);
  readonly disabled = input(false);
  readonly result = output<WriterResult>();
  readonly progress = output<Parameters<NonNullable<WriterRunOptions['onProgress']>>[0]>();
  readonly requestError = output<unknown>();
  readonly error = model('');
  readonly api;
  readonly resultText = computed(() => this.api.current().output);
  readonly checkAvailability = (options: object) => this.api.requestAvailability(options as WriterCreateCoreOptions);
  constructor(destroyRef: DestroyRef) {
    this.api = createAngularWriterWorkflow(destroyRef);
  }
  async execute(
    value: string,
    ...
    try {
      ...
      await this.api.writeStreamingToText(value, options);
